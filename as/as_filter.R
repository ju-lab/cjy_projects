...
data = read.table('/home/users/team_projects/AnkylosingSpondylitis/analysis/freebayes/vcf/annovar_files/B27NE_char.txt.fm.txt', sep='\t', header=T, col.names = c('CHROM', 'POS','REF','ALT', 'sample' ,'sample_IDs', 'gene', 'mut_type', 'exac_tot', 'exac_eas', 'exac_max', 'Kova', 'odds_tot', 'odds_eas', 'odds_max', 'odds_kova', 'Father', 'Mother', 'Proband', 'Sibling'))
data = as.tibble(data)
...
for(chromosome in 1:23){
  ...
  filtered_data = data  %>% replace_na(list(odds_max= 1000)) %>% 
    ...
    filter(BEFOREDIST > 100, AFTERDIST>100, Mother==F, Sibling==F)

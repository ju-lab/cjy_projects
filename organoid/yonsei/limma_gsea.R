...
glMDPlot(tfit, coef=1, status=dt, main=colnames(tfit)[1], side.main="ENTREZID", counts=x$counts, group=group, launch=T)


bem_vs_mat.topgenes <- bem_vs_mat$ENTREZID[1:100]
i <- which(v$genes$ENTREZID %in% bem_vs_mat.topgenes)
mycol <- colorpanel(1000, 'blue', 'white', 'red')
pdf('/home/users/cjyoon/Projects/yonsei_organoid/limma/ready_files/top_degenes.pdf')
...
cam.bem_vs_mat <- camera(v, idx, design, contrast=contr.matrix[,1])
View(cam.bem_vs_mat)
cam.bem_vs_mat_write = as.tibble(cam.bem_vs_mat) %>% mutate(HALLMARK = row.names(cam.bem_vs_mat)) # added rowname as a separate column for writing

write.table(cam.bem_vs_mat_write, '/home/users/cjyoon/Projects/yonsei_organoid/limma/ready_files/gsea/cam_bem_vs_mat.txt', sep='\t', quote=F, row.names=F, col.names=T)
pdf('/home/users/cjyoon/Projects/yonsei_organoid/limma/ready_files/gsea/significant_barcodeplot.pdf')
barcodeplot(efit$t[,1], index=idx$HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION, index2=idx$HALLMARK_COAGULATION, main="BEM_vs_MAT")
barcodeplot(efit$t[,1], index=idx$HALLMARK_ANDROGEN_RESPONSE, index2=idx$HALLMARK_TGF_BETA_SIGNALING, main="BEM_vs_MAT")
